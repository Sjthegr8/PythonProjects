...
#
# Computer Project 06
# this script is very tough and is basically a database of various zoos
# the main function prints the Menu (choices variable) and prompts for a choice. Then,
# •f the input is 1,2, 3, or 4:
# It outputs the related task options as will be detailed.
# And then, reprint the Menu.
# prompts for a choice.
# If the input is x or X, the program displays a Thank you message and quits.
# If the input is not one of the valid choices (any other string):
...
def get_zoo_file_names(zoo_file_count):
    ...
    i read the zoo file
    input zoo name
    return a list of zoo files
    :param zoo_file_count:
    :return:
    """
    zoo_name_list = []
    zoo_files_dict = {}
...
def calculate_cost_of_each_zoo(zoo_name_list, zoo_files_dict, expenses_data,
species_list, other_expenses, workers_cost):
    """

    :param zoo_name_list:
    :param zoo_files_dict:
    :param expenses_data:
    :param species_list:
    ...
    most_expensive_zoos = [zoo_expense[0].capitalize() for zoo_expense in
    zoo_expense_results if zoo_expense[1] == max_expense]
...
def open_zoo_files(zoo_files_dict):
    """
    opening zoo files
    return a dictionary with zoo files
    help in zoology
    :param zoo_files_dict:
    :return:
    """
...
def dict_new(zoo_file_handles, species_list, zoo_name_list):
    ...
    while x < len(zoo_name_list):
        ...
        for line in file:
            ...
            line[5] == '' or line[6] == '' or line[7] == '' or line[8] == '':
                ...
                continue
            if [line[1],line[2],line[3],line[4],line[5],line[6],line[7],line[8]] in name_li:
                continue
            else:
                name_li.append([line[1],line[2],line[3],line[4],line[5],line[6],line[7],line[8]])
            species_id = line[0]
            name = line[1]
            age = line[2]
            sex = line[3]
        ...
        if len(details_list) > 1:
            for i in range(len(details_list)):
                for j in range(i + 1, len(details_list)):
                    animal_one = details_list[i]
                    animal_two = details_list[j]
                    pair = [animal_one, animal_two]
    ...
    if pairs == []:
        print("There are no twins in the zoo.")
    ...
    while True:
        ...
        if user_input == "1":
            ...
            for zoo_name in zoo_name_list:
                file = open(zoo_files_dict[zoo_name], 'r')
                sick_counter, average, sickest_species, species_id_map = (
                calculate_sick(species_dict, file,species_list))
                if sickest_species:
                    print(
                        f"{average}% of {zoo_name.capitalize()} Zoo animals are sick. "
                        f"The sickest being {sickest_species}.")
                else:
            ...
            calculate_cost_of_each_zoo(zoo_name_list, zoo_files_dict,
            expenses_data, species_list, other_expenses,workers_cost)
        elif user_input == "3":
